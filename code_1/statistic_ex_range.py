...
def save_extreme_records(extreme_records, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad):
    # 创建标签专属子文件夹
    save_dir = os.path.join(base_save_dir, target_label)
    ...
    logging.info(f"📝 已保存日期 {date_ad} 标签 '{target_label}' 极端值TXT：{extreme_txt_path}")

def save_extreme_points_df(extreme_points_df, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad):
    if extreme_points_df.empty:
        logging.info(f"日期 {date_ad} 标签 '{target_label}' 没有极端点数据，跳过CSV保存")
        return
    ...
    logging.info(f"💾 已保存日期 {date_ad} 标签 '{target_label}' 极端点完整数据CSV：{extreme_csv_path}")

def plot_extreme_distribution(channel_cols, channel_extreme_counts, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad):
    if np.sum(channel_extreme_counts) == 0:
        logging.warning(f"日期 {date_ad} 标签 '{target_label}' 没有极端值，跳过绘图")
        return
    ...
    plt.close()
    logging.info(f"📊 已生成日期 {date_ad} 标签 '{target_label}' 极端值分布图：{save_path}")

def main(threshold_phases, extreme_pos, extreme_neg):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    setup_logging(script_dir)
    
    labels_dir = "E:\\A_exploring_clouds\\data\\test_3_days\\original_data"  # 修改为用户指定的原始数据目录
    base_save_dir = "E:\\A_exploring_clouds\\data\\test_3_days\\statistic"  # 修改为用户指定的分类结果目录
    ...
    for threshold_clear in threshold_clears:
        ...
        try:
            ...
            for date_ad, file_path in file_info_list:
                ...
                logging.info(f"✅ 日期 {date_ad} 发现独特标签：{list(unique_labels)}")
                ...
                for target_label in unique_labels:
                    ...
                    save_extreme_records(extreme_records, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad)
                    save_extreme_points_df(extreme_points_df, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad)
                    plot_extreme_distribution(channel_cols, channel_extreme_counts, base_save_dir, target_label, threshold_clear, threshold_phases, extreme_pos, extreme_neg, date_ad)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FCDI极端值分析与绘图程序（针对所有标签，扩大极端范围，按日期分别处理）")
