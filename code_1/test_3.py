...
def load_fcdi_data(fcdi_csv_path, channel_pairs):
    ...
    if total_columns < 11 + 230:
        raise ValueError(f"FCDI数据列数不足，实际{total_columns}列")
    
    # 加载所有列，不筛选FCDI列（保持原FCDI不变）
    fcdi_df = fcdi_all.copy()
    
    # 为所有列分配列名：前11列固定，剩余为channel_1到channel_230
    channel_cols_all = [f'channel_{i}' for i in range(1, total_columns - 10)]
    fcdi_df.columns = required_cols + channel_cols_all
    ...
    print(f"✅ 加载FCDI数据（共{len(fcdi_df)}个点），总列数{total_columns}（包含所有230个通道对），用于可视化的匹配通道对{len(channel_cols)}个")
...
def save_new_csv(fcdi_df, fcdi_csv_path):
    ...
    print(f"✅ 新CSV保存至：{new_csv_path}（包含所有原始FCDI列 + label）")
    return new_csv_path
