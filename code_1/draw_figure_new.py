...
def main():
    ...
    plot_mode = "single"  # 可选："single" 或 "all"
    column_offset = -2  # 已验证偏移正确
    ...
    try:
        ...
        # Step 1: 随机抽样1000行（若总行数小于1000，则全部取）
        total_rows = len(fcdi_df)
        sample_size = min(1000, total_rows)
        fcdi_sampled = fcdi_df.sample(n=sample_size, random_state=42).reset_index()  # 保留原始行号
        print(f"\n🎯 已随机抽取 {sample_size} 条数据（共 {total_rows} 条）用于绘图")

        # Step 2: 导出抽样信息（行号 + 经纬度）
        ...
        with open(sample_info_path, "w", encoding="utf-8") as f:
            f.write("原始行号\t经度(alon1)\t纬度(alat1)\n")
            for _, row in fcdi_sampled.iterrows():
                f.write(f"{int(row['index'])}\t{row['alon1']:.4f}\t{row['alat1']:.4f}\n")
        print(f"📝 抽样信息已保存至：{sample_info_path}")

        # 使用抽样数据绘图
        if plot_mode == "single":
            print(f"\n开始生成每个点的散点图（共{len(fcdi_sampled)}个样本）...")
            for _, row in fcdi_sampled.iterrows():
                alon = row['alon1']
                alat = row['alat1']
            ...
            print(f"\n开始生成所有点的散点图（样本数：{len(fcdi_sampled)}）...")
            plot_all_points(fcdi_sampled, height_df, channel_cols, save_all_path)
